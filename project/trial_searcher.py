...
def read_indexing_records(file_path: str) -> pandas.DataFrame:
...
def find_duchenne_muscular_dystrophy_conditions(client: openai.OpenAI, unique_conditions: List[str]) -> List[str]:
    ...
    for i in range(0, len(unique_conditions), batch_size):
        ...
        I need you to analyze a list of diseases and identify the ones that are Duchenne Muscular Dystrophy, considering various naming conventions.
        Please analyze this list of medical conditions and return ONLY a JSON array containing the conditions:
        ...
        Only include conditions that are  Duchenne Muscular Dystrophy itself (various naming conventions). 
        For example, "{', '.join(example_list)}", etc.

        If no conditions in this list are related to Duchenne Muscular Dystrophy, return an empty array: []
        """
        # Make the API call
        response = client.chat.completions.create(
            model="gpt-4",
            messages=[
                {"role": "system", "content": "You are a medical expert specializing in muscular dystrophies and neuromuscular disorders."},
    ...
    if len(all_related_conditions) == 0:
        print("No conditions related to Duchenne Muscular Dystrophy found in any batch.")
        all_related_conditions = example_list

    logger.info(f"Found {len(all_related_conditions)} conditions related to Duchenne Muscular Dystrophy: {', '.join(all_related_conditions)}")
    return all_related_conditions


def find_common_eligibility_criteria(client: openai.OpenAI, relevant_trials: List[str], trial_data: List[dict]) -> str:

    # filter trial_data to only include trials with NCT IDs in relevant_trials
    relevant_trial_data = [trial for trial in trial_data if trial.get('nct_id') in relevant_trials]
    ...
    response = client.chat.completions.create(
        ...
        messages=[
            {
                "role": "system", 
                "content": "You are a clinical research expert analyzing clinical trial data. Focus on providing specific, actionable information about eligibility criteria."
            },
            {
                "role": "user", 
                "content": f"""Within the disease "Duchenne muscular dystrophy", what are common eligibility criteria? 
                ...
                Focus on the NCT IDs we found: {', '.join(relevant_trials)}.
                
                The eligibility criteria for each trial can be found in the following trial data: 
                {relevant_trial_data}"""
            }
        ],
        max_tokens=1000,
    ...
    logger.info("Common eligibility criteria for Duchenne Muscular Dystrophy trials:")
    logger.info(response.choices[0].message.content)


def find_trials_per_condition(conditions: List[str], indexing_records: pandas.DataFrame) -> List[str]:
    relevant_trials = indexing_records[indexing_records['a.alias'].isin(conditions)]['s.id'].unique().tolist()
    logger.info(f"Found {len(relevant_trials)} trials related to conditions '{', '.join(conditions)}'.")
...
def find_trials_per_patient(patient_data: dict, indexing_records: pandas.DataFrame) -> List[str]:       
...
def determine_eligibility_score(client: openai.OpenAI, patient_profile: str, eligibility_criteria: dict) -> float:
...
def determine_eligibility_per_trial(client: openai.OpenAI, indexing_records: pandas.DataFrame, trial_data: List[dict]):
    ...
    for patient_path in patient_paths:
        ...
        relevant_trials = find_trials_per_patient(patient_data, indexing_records)
        ...
        for trial_id in relevant_trials:
            ...
            eligibility_score = determine_eligibility_score(client, patient_profile, eligibility_criteria)
            trial_scores[trial_id] = eligibility_score
            logger.debug(f"Eligibility score for trial {trial_id}: {eligibility_score}")

        logger.info(f"Trial scores for this patient {trial_scores}")
        # eligibility_score = determine_eligibility_score(client, patient_data,relevant_trials, trial_data)
...
trial_data = read_payloads_jsonl(TRIAL_INFORMATION_PATH)

client = initiate_openai_client()
indexing_records = read_indexing_records(INDEXING_RECORDS_PATH)

# Get unique conditions to analyze if they are related to Duchenne muscular dystrophy
unique_conditions = indexing_records['a.alias'].unique().tolist()

relevant_conditions = find_duchenne_muscular_dystrophy_conditions(client, unique_conditions)
relevant_trials = find_trials_per_condition(relevant_conditions, indexing_records)
eligibility_criteria = find_common_eligibility_criteria(client, relevant_trials, trial_data)
logger.info(f"Common eligibility criteria for Duchenne Muscular Dystrophy trials:\n{eligibility_criteria}")

determine_eligibility_per_trial(client, indexing_records, trial_data)
